...
%              Performan_PA(p_idx+n-1,8)=PAE;
        ...
        data(f_idx).table(p_idx,3) = p_out_mean; % power meter out
        data(f_idx).table(p_idx,4) = Pout;       % PA_out
        data(f_idx).table(p_idx,5) = Gain;     % gain
        data(f_idx).table(p_idx,6) = I_m;      % main current
        data(f_idx).table(p_idx,7) = PAE;      % PAE
        data(f_idx).table(p_idx,8) = DE;      % drain efficiency   
...
for freq=f_start:f_step:f_stop
    ...
    for p_target=p_min:p_step:p_max
        p_idx = 1+(p_target-p_min)/p_step;
        Id_real(p_idx,f_idx)=data(f_idx).table(p_idx,6);
        Pout_real(p_idx,f_idx)=data(f_idx).table(p_idx,3);   
        Gain_real(p_idx,f_idx)=data(f_idx).table(p_idx,5);
        Pin_real(p_idx,f_idx)=data(f_idx).table(p_idx,2);
        DE_real(p_idx,f_idx)=data(f_idx).table(p_idx,8);
        PAE_real(p_idx,f_idx)=data(f_idx).table(p_idx,7);  
