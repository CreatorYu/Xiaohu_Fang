...
for freq=f_start:f_step:f_stop
    ...
    for p_target=p_min:p_step:p_max
       ...
       %  Pin=Mea_driver_MMWave(f_start,f_stop,f_idx,p_idx)+Mea_Probe(freq)+Short_line(freq);
       Pin=p_target;     
       SignalGenerator_SMW200A(SMWAdd,freq,p_target,1);
           ...
           % Pdc=PowerSupply_DP821A(DP821A_IP);
            Pdc=28*(I_m-0.001);
...
data_file_name = [ 'PA_data_CW' datestr(now,'yyyy-mm-dd-HH-MM' )];
...
% 一下code用于plot paper中的图
Nx=7;
linestyle1=char('-ro', '-m*', '-c^', '-bv', '-gs', '-y>', '-k<','-r*', '-m^', '-cv');
figure(10)
hold on
for i=1:N2
...
hold on
for i=1:N2
    plot(Pout_real,Gain_real,'linewidth',2);
end
set(gcf,'color','w');
axis([10,30,0,12]);
...
hold on
for i=1:N2
    plot(Pout_real,DE_real,'linewidth',2);
end
set(gcf,'color','w');
axis([10,30,0,60]);
% h=legend('23GHz','24GHz','25GHz','26GHz','27GHz','28GHz','29GHz');
