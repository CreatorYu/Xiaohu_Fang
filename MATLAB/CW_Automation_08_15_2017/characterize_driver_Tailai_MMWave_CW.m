...
for freq=f_start:f_step:f_stop
    ...
    for p_target=p_min:p_step:p_max
        p_idx = 1+(p_target-p_min)/p_step;
           ...
           %p_out = fun_MATLAB_directSCPI_NRPZxx_Trace_my(freq,100e-6,10e-6,1e-5);
          
            p_out_mean(p_idx,f_idx)=p_out
            fprintf('p_out is %2.4f ',p_out_mean);
        S21(p_idx,f_idx)=fun_VNA_S21_CW_read(VNA_addr);
        mea_Pout(p_idx,f_idx)=p_out_mean(p_idx,f_idx)-Mea_Attenuator_MMWave_2023_1_11(freq);
        Gain(p_idx,f_idx)=p_out_mean(p_idx,f_idx)-p_in-Mea_Attenuator_MMWave_2023_1_11(freq);
...
data_file_name = [ 'Driver_MMWave_PA_data_CW' datestr(now,'yyyy-mm-dd-HH-MM' )];
