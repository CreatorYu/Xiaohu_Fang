function SignalGenerator_SMW200A_two_tone(SMWAdd,center_frequency,spacing_frequency,input_power,i)

% SMWAdd='TCPIP::192.168.1.101::INSTR';
% center_frequency=26;
% spacing_frequency=40e6;
% input_power=-29;
...
elseif i==1 %i=1,Set the RF parameter of SMW200A
  %Set the RF parameter of SMW200A
    ...
    [Status]=rs_send_command( InstrObject, 'SOURce1:BB:MCCW:CARRier:COUNt 2');
    if( ~status ); clear;  return; end

    [Status]=rs_send_command( InstrObject, ['SOURce1:BB:MCCW:CARRier:SPACing ' num2str(spacing_frequency)]);
    if( ~status ); clear;  return; end

    [Status]=rs_send_command( InstrObject, 'SOURce1:BB:MCCW:STATe 1');
    if( ~status ); clear;  return; end
