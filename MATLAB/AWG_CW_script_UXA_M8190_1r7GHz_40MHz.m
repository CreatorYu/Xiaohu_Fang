...
PM.offset(ATN.attenuation(Fcarrier));
...
In_I_AWG = resample(In_I, UpsampleTx, DownsampleTx, 500);
In_Q_AWG = resample(In_Q, UpsampleTx, DownsampleTx, 500);
...
AWG_M8190A_DAC_Amplitude(1,0.61);
AWG_M8190A_MKR_Amplitude(1,1.5);
...
[RecI_captured, RecQ_captured] = IQCapture_UXA (Fcarrier, FsampleRx/1.25, FrameTime, UXAAdd, UXA_Atten, UXA_ClockReference);
 ...
 PlotSpectrum(ResampledRecI, ResampledRecQ,ResampledRecI, ResampledRecQ,1,FsampleRx);
% ResampledRecI = RecI_captured(200:end);
% ResampledRecQ = RecQ_captured(200:end);
%
[UpsampleTx, DownsampleTx] = rat(FsampleTx/FsampleRx);
    ResampledRecI = resample(ResampledRecI,UpsampleTx,DownsampleTx,500);
    ResampledRecQ = resample(ResampledRecQ,UpsampleTx,DownsampleTx,500);
     PlotSpectrum(ResampledRecI, ResampledRecQ,ResampledRecI, ResampledRecQ,1,FsampleTx);
...
%     PlotSpectrum(In_I, In_Q,ResampledRecI, ResampledRecQ,1,FsampleTx);
  %  
    ...
    sig  = complex(DelayAdjusted_Out_I(1:25000), DelayAdjusted_Out_Q(1:25000));
    ps(sig, FsampleTx)
    sig_nospurs = remove_spurious_specific(sig, FsampleTx, [-50e6 0]);
    ps(sig_nospurs, FsampleTx)
...
% AWG_M8190A_Output_OFF(1);   % measurement end
