...
%     PlotSpectrum(In_I, In_Q,ResampledRecI, ResampledRecQ,1,FsampleTx);
  %  
    ...
    PlotSpectrum(DelayAdjusted_In_I, DelayAdjusted_In_Q, DelayAdjusted_Out_I, DelayAdjusted_Out_Q,10,FsampleTx);
    
%     sig  = complex(DelayAdjusted_Out_I(1:8000), DelayAdjusted_Out_Q(1:8000));
%     ps(sig, FsampleTx)
%     sig_nospurs = remove_spurious_specific(sig, FsampleTx, [-50e6 0]);
%     ps(sig_nospurs, FsampleTx)
...
%     PlotSpectrum(In_I, In_Q,ResampledRecI_ACP, ResampledRecQ_ACP,10,FsampleTx);
    [freq, spectrum] = Calculated_Spectrum(DelayAdjusted_Out_I, DelayAdjusted_Out_Q,FsampleTx);
Nf=length(freq); delta_f=FsampleTx/Nf;  
y1=-FBB; y2=FBB; y3=-3*FBB; y4=3*FBB;
    x1=round(1024+y1/delta_f);
    x2=round(1024+y2/delta_f);
    x3=round(1024+y3/delta_f);
    x4=round(1024+y4/delta_f);
    Fund_L=max(spectrum((x1-20):(x1+20)))
    Fund_R=max(spectrum((x2-20):(x2+20)))
    IM3_L=max(spectrum((x3-20):(x3+20)))
    IM3_R=max(spectrum((x4-20):(x4+20)))
    IMD3_L=IM3_L-Fund_L;
    IMD3_R=IM3_R-Fund_R;
...
%     display([ 'EVM          = ' num2str(EVM_perc)      ' % ' ]);
    display([ 'IMD3 (L/R)   = ' num2str(IMD3_L) ' / '  num2str(IMD3_R) ' dB ' ]);
