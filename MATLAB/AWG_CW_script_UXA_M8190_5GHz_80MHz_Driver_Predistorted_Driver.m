...
time_now = clock;
dir_name = strcat('5r0GHz',date,'_',int2str(time_now(4)),'_',int2str(time_now(5)),'_',int2str(time_now(6)));
UXA_Atten             = 6;    
%
%
%
...
InI_Predist_path = 'I_Input_PreDist_5r0.txt';
InQ_Predist_path = 'Q_Input_PreDist_5r0.txt';
BW = 80e6;
fG = 600e3;
PAPR_limit = 11;
FsampleSig=1e9;
...
FrameTime = 0.128e-3;
%
In_I_beforeDPD=load(InI_beforeDPD_path);
In_Q_beforeDPD=load(InQ_beforeDPD_path);
In_I=load(InI_Predist_path);
In_Q=load(InQ_Predist_path);
[UpsampleSig, DownsampleSig] = rat(FsampleTx/FsampleSig);
In_I = resample(In_I, UpsampleSig, DownsampleSig, 500);
In_Q = resample(In_Q, UpsampleSig, DownsampleSig, 500);
PlotSpectrum(In_I, In_Q,In_I, In_Q,1,FsampleTx);
%
In_I_beforeDPD = In_I_beforeDPD(1:round(FrameTime*FsampleTx));
In_Q_beforeDPD = In_Q_beforeDPD(1:round(FrameTime*FsampleTx));
...
%     PlotSpectrum(In_I, In_Q,ResampledRecI, ResampledRecQ,1,FsampleTx);
  %  
    ...
    In_I=In_I_beforeDPD; In_Q=In_Q_beforeDPD;
