...
load('driver_data_TL1G18G2023-02-18-17-35.mat');
f_start_D =3.8e9;
f_stop_D =6.4e9;
%
p_min_D = -40;
...
for freq=f_start:f_step:f_stop
    ...
    for p_target=p_min:p_step:p_max
        ...
        Pin=Mea_driver_sub6G(f_start,f_stop,f_idx,p_idx)+Mea_Probe(freq);
        ...
        SignalGenerator_SMW200A(SMWAdd,freq,p_target,1);
            ...
            if p_target < -30
                ...
                tri = 'TRIGGER:A:LEVEL:CH1 0.042';
            elseif (p_target >= -30) && (p_target < -25)
                scale1 = 'CH1:SCALE 0.05';
                tri = 'TRIGGER:A:LEVEL:CH1 0.055';    
            else
                scale1 = 'CH1:SCALE 0.1';
                tri = 'TRIGGER:A:LEVEL:CH1 0.08';
            end
...
%              V_m=DC(1);
 ...
 data_file_name = [ 'PA_5_8G_data_Pulse_' datestr(now,'yyyy-mm-dd-HH-MM' )];
...
set(gcf,'color','w');
axis([20,40,0,80]);
h=legend('5GHz','5.1GHz','5.2GHz','5.3GHz','5.4GHz','5.5GHz','5.6GHz','5.7GHz','5.8GHz','5.9GHz','6GHz');
