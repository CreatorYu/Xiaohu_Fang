...
%Device Adress
SMWAdd='TCPIP::192.168.1.111::INSTR';   % set the address of signal generator
...
% load('driver_data_for_SLCG_2G_3G_2023-04-10-13-30.mat');
% f_start_D =2e9;
% f_stop_D =3e9;
% %
% p_min_D = -35;
...
for freq=f_start:f_step:f_stop
    ...
    elseif freq > 2.1e9 && freq <= 2.4e9
        Voltage_Setting_DP821A(DP821A_IP,4.6,2);
    elseif freq > 2.4e9 && freq <= 2.7e9
        Voltage_Setting_DP821A(DP821A_IP,4.9,2);
    elseif freq > 2.7e9 && freq <= 3.0e9
        Voltage_Setting_DP821A(DP821A_IP,4.9,2);
    elseif freq > 3.0e9 && freq <= 3.3e9
        Voltage_Setting_DP821A(DP821A_IP,4.9,2);
    else
        Voltage_Setting_DP821A(DP821A_IP,5.2,2);
     end
%     if freq < 1.3e9
%         Voltage_Setting_DP821A(DP821A_IP,4.4,2);
%     elseif freq > 1.3e9 && freq <= 1.5e9
%         Voltage_Setting_DP821A(DP821A_IP,4.4,2);
%     elseif freq > 1.5e9 && freq <= 1.8e9
%         Voltage_Setting_DP821A(DP821A_IP,4.8,2);
%     else 
%         Voltage_Setting_DP821A(DP821A_IP,4.8,2);
%      end
    ...
    %PM.offset(0);
     ...
     Pin_driver_max=fun_Driver_out2in(Pout_driver_max,freq); 
    ...
    for p_target=p_sg_min:p_step:p_sg_max
        ...
        Pin=fun_Driver_in2out(p_target, freq)-1;
        % Pin=p_target+32;     
        if Conditon==1
           SignalGenerator_SMW200A(SMWAdd,freq,p_target,1);
        else 
            SignalGenerator_SMW200A(SMWAdd,freq,p_target0,1);
        end
%             p_in=Pin;
%             p_sg=p_target;
            %
        if Conditon==1
           SignalGenerator_SMW200A(SMWAdd,freq,p_target,2);
            ...
            SignalGenerator_SMW200A(SMWAdd,freq,p_target0,2);
        end
            fprintf('p_sg is %2.4f ',p_target);
            fprintf('p_in is %2.4f ',Pin);
...
%             p_out = MATLAB_directSCPI_NRPZxx_Avg_Power_my(freq);
            fprintf('p_out from PM is %2.4f ',p_out);
             ...
             if p_target > -45 && p_target <-30
                 scale1='0.1';
                 scale2='0.1';
                 tri='0.09';
             elseif p_target >= -30 && p_target <-25
                 tri='0.12';
             end

            I_m=fun_Main_TI_Osc(scale1,tri,ch1);
            pause(0.5);
            I_a=fun_Main_TI_Osc(scale2,tri,ch2);
            if I_m>0.3 
                 scale1='0.2';
                 tri='0.25';
            elseif I_a>0.3 
           ...
           %DC= PowerSupply_DP932U(DP932U_IP);
             ...
             %I_a=DC(2);
%              if Conditon==1 && I_m<0.01 && p_target>-18
...
%              end
            ...
            Pout=p_out-Attenuation(f_idx)+0.3;
            fprintf('SG power is %02.2f ', Pout);
            gain=Pout-Pin;
            P_DC=16*(I_m-0.001)+28*(I_a-0.001);
            DE=100*10^((Pout-30)/10) / P_DC;
            PAE=100*(10^((Pout-30)/10)-10^((Pin-30)/10))/ P_DC;
            
            % create the data table
        data(f_idx).table(p_idx,1) = p_target;     % signal generator power level
        data(f_idx).table(p_idx,2) = Pin;     % PA_in (a.k.a Driver_out)
        ...
        if Conditon==1
           SignalGenerator_SMW200A(SMWAdd,freq,p_target,3);
            ...
            SignalGenerator_SMW200A(SMWAdd,freq,p_target0,3);
        end
            ...
            pause(0.5);
            I_m=fun_Main_TI_Osc('0.1','0.05',ch1);
            I_a=fun_Main_TI_Osc('0.1','0.05',ch2);
            pause(0.5);
        % create the data table
        %mea_P(f_idx)=mean(Atten);
       
    end
%  data_file_name = [ 'SLCG_2_data_pulse_3_1G_3_6G_16V' datestr(now,'yyyy-mm-dd-HH-MM' )];
...
set(gcf,'color','w');
axis([20,42,0,80]);
h=legend('2.4GHz','2.4GHz','2.6GHz','2.8GHz','3.0GHz');
