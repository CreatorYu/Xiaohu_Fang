...
for k=1:6
    if k==1
        load('MMwave_DPA_24_5GHz_CW.mat')
    elseif k==2
        load('MMwave_DPA_25GHz_CW.mat');
    elseif k==3
        load('MMwave_DPA_25_5GHz_CW.mat');
    elseif k==4
        load('MMwave_DPA_26GHz_CW.mat');
    elseif k==5
        load('MMwave_DPA_26_5GHz_CW.mat');
    elseif k==6
        load('MMwave_DPA_27GHz_CW.mat');     
    end
driver_array=phase_driver(:,k+1);
AMPM_both(:,k)=Phase_PA_Driver;
AMPM_both2(:,k)=Phase_PA_Driver-Phase_PA_Driver(1);
% 
AMPM_PA(:,k)=AMPM_both(:,k)-driver_array;
AMPM_PA2(:,k)=AMPM_PA(:,k)-AMPM_PA(1,k);
