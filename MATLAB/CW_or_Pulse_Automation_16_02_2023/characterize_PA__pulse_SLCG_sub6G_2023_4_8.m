...
load('PA_SLCG_data_pluse_Im2023-04-12-18-02.mat')
Im=Im_real;
...
for freq=f_start:f_step:f_stop
    ...
    elseif freq >= 2.2e9 && freq < 2.5e9
        fun_PowerSetting_HMP2030(HMP2030_IP,HMP_channel,4.8);
    elseif freq >= 2.5e9 && freq < 2.7e9
        ...
        SignalGenerator_SMW200A(SMWAdd,freq,p_target,1);
            ...
            %I_m = fun_DS2102A(p_idx);
             ...
             if p_target < -30
                 ...
                 scale2 = 'CH2:SCALE 0.1';
                tri = 'TRIGGER:A:LEVEL:CH1 0.085';
            elseif (p_target >= -30) && (p_target < -25)
                 scale1 = 'CH1:SCALE 0.1';
                 scale2 = 'CH2:SCALE 0.1';
                tri = 'TRIGGER:A:LEVEL:CH1 0.105';    
            else
                 scale1 = 'CH1:SCALE 0.1';
                 scale2 = 'CH2:SCALE 0.1';
                 tri = 'TRIGGER:A:LEVEL:CH1 0.175';   
            end
             
             pause(0.5);
            I_m=Im(p_idx,f_idx);
            I_a=fun_Main_TBS_Osc_ch(scale1,tri);
           % I_a=fun_Main_TBS_Osc_ch2(scale2,tri);
           %[~,I_m] = fun_PowerSupply_N6705A_Lan(Lan_addr,chan);
          
           %DC= PowerSupply_DP932U(DP932U_IP);
          
             
           %  I_a =fun_Main_TBS_Osc(scale1,tri);
            
            Pout=p_out-Mea_Attenuator_2(freq);
            fprintf('SG power is %02.2f ', Pout);
            gain=Pout-Pin;
            P_DC=14*(I_m-0.001)+28*I_a;
 ...
 data_file_name = [ 'PA_SLCG_data_pluse' datestr(now,'yyyy-mm-dd-HH-MM' )];
...
set(gcf,'color','w');
axis([20,42,0,80]);
h=legend('2.1GHz','2.2GHz','2.3GHz','2.4GHz','2.5GHz','2.6GHz','2.7GHz','2.8GHz','2.9GHz','3GHz');
