...
load('driver_data_SBP2023-04-07-11-21.mat');
f_start_D =24e9;
f_stop_D =29.5e9;
%
p_min_D = -35;
...
for freq=f_start:f_step:f_stop
    ...
    for p_target=p_min:p_step:p_max
        ...
        Pin=Pout_Driver(p_idx,f_idx)+Mea_Probe(freq);
        ...
        SignalGenerator_SMW200A(SMWAdd,freq,p_target,1);
            ...
            elseif (p_target >= -25) && (p_target < -20)
                scale1 = 'CH1:SCALE 0.02';
                tri = 'TRIGGER:A:LEVEL:CH1 0.035';    
            else
                scale1 = 'CH1:SCALE 0.05';
                tri = 'TRIGGER:A:LEVEL:CH1 0.45';
            end
...
%              V_m=DC(1);
            ...
            Pout=p_out-Mea_Attenuator_MMWave(freq/1e9);
 ...
 data_file_name = [ 'PA_mmwave_single_data_Pulse_' datestr(now,'yyyy-mm-dd-HH-MM' )];
