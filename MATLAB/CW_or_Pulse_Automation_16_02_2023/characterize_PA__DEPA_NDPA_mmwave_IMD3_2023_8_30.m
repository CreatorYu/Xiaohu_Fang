...
for freq=f_start:f_step:f_stop
    ...
    %PM.offset(0);
     ...
     Conditon=1; p_target0=-18;
          SignalAnalyzer_FSW43_IMD3(FSWIP,freq,spacing,RefLev,0);
    for p_target=p_sg_min:p_step:p_sg_max
        ...
        % Pin=p_target+32;     
        SignalGenerator_SMW200A_two_tone(SMWAdd,freq,spacing,p_target,1);
            p_in=Pin;
            p_sg=p_target;
            SignalGenerator_SMW200A_two_tone(SMWAdd,freq,spacing,p_target,2);
            ...
            [IMD3_lower,IMD3_upper]=SignalAnalyzer_FSW43_IMD3(FSWIP,freq,spacing,RefLev,1);
        ...
        data(f_idx).table(p_idx,3) = Pout;
        data(f_idx).table(p_idx,4) = IMD3_lower;      
        data(f_idx).table(p_idx,5) = IMD3_upper;   
        data(f_idx).table(p_idx,6) = Gain;
    end
            SignalGenerator_SMW200A_two_tone(SMWAdd,freq,spacing,p_target,3);
...
%             pause(1);
 ...
 data_file_name = [ 'PA_data_DEPA_IMD3_spacing_200M_24G_31G_' datestr(now,'yyyy-mm-dd-HH-MM' )];
...
ylabel('IMD3 lower (dBc)','fontsize',15,'fontname','Times New Roman','fontweight','b');    
hold off    
grid on
% 平时简单的绘图如下
figure()
hold on
for i=1:Nf
    plot(data(i).table(:,3),data(i).table(:,5),linestyle1(i,:),'linewidth',1)
end
% for i=1:Nx
%     plot(Pout_dB_mark(:,i),eff_mark(:,i),linestyle1(i,2:3),'linewidth',2);
%     plot(Pout_dB(:,i),eff(:,i),linestyle1(i,1:2),'linewidth',2);
...
% title('DE vs Normalized Pout for various x_n','fontsize',14,'fontname','Times New Roman');
xlabel('Output Power (dBm)','fontsize',15,'fontname','Times New Roman','fontweight','b');
ylabel('IMD3 upper (dBc)','fontsize',15,'fontname','Times New Roman','fontweight','b');    
hold off    
grid on
...
hold on
for i=1:Nf
    plot(data(i).table(:,3),data(i).table(:,6),linestyle1(i,:),'linewidth',1)
end
% for i=1:Nx
%     plot(Pout_dB_mark(:,i),eff_mark(:,i),linestyle1(i,2:3),'linewidth',2);
%     plot(Pout_dB(:,i),eff(:,i),linestyle1(i,1:2),'linewidth',2);
