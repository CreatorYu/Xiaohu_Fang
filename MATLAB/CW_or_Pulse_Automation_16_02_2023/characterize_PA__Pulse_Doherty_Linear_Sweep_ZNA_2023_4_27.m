...
load('driver_data_for_Doherty_4_6G_6_2G_2023-04-27-16-22.mat')
f_start_D =4.6e9;
f_stop_D =6.2e9;
%
...
Phase_Driver=phase_S21_Driver(p1:p2,f1:f2);
...
%         fun_PowerSetting_HMP2030(HMP2030_IP,HMP_channel,3.3); %Idsm=59.9mA
%     elseif Fcarrier ==5.4e9
%         fun_PowerSetting_HMP2030(HMP2030_IP,HMP_channel,3.34); %Idsm=59.9mA
...
%         fun_PowerSetting_HMP2030(HMP2030_IP,HMP_channel,4.52); %Idsm=31.7mA
%     else
%         fun_PowerSetting_HMP2030(HMP2030_IP,HMP_channel,4.89);%Idsm=16.6mA
% end
%
    ...
    for p_target=p_min:p_step:p_max
        ...
        % Pin=p_target+32;     
           ...
           %DC= PowerSupply_DP932U(DP932U_IP);
            ...
            if (p_target >= -30) && (p_target < -25)
                scale1 = 'CH1:SCALE 0.1';
                tri = 'TRIGGER:A:LEVEL:CH1 0.060';
            elseif (p_target >= -25) && (p_target < -17)
                scale1 = 'CH1:SCALE 0.1';
                tri = 'TRIGGER:A:LEVEL:CH1 0.120';    
            else
                scale1 = 'CH1:SCALE 0.2';
                tri = 'TRIGGER:A:LEVEL:CH1 0.200';
            end
             
             pause(0.5);
            I_m=fun_Main_TBS_Osc(scale1,tri);
            I_a=I_m;  %regrect Ia  Im=Im_real+Ia_real

            Pout=p_out-Mea_Attenuator_2(freq);
            Gain_total=Pout-p_sg;
            fprintf('SG power is %02.2f ', Pout);
            gain=Pout-Pin;
            P_DC=28*(I_m);
            DE=100*10^((Pout-30)/10) / P_DC;
            PAE=100*(10^((Pout-30)/10)-10^((Pin-30)/10))/ P_DC;
            phase_S21=fun_VNA_S21_phase_read(VNA_addr); %read the S21 from ZNA
        ...
        data(f_idx).table(p_idx,12) = phase_S21;      % phase_PA_and_driver
        data(f_idx).table(p_idx,13) = Gain_total;      % Gain_PA_and_driver
    end
 ...
 data_file_name = [ 'PA_Doherty_linear_data_Pulse_5_4_' datestr(now,'yyyy-mm-dd-HH-MM' )];
...
Pout_real_PA=zeros(Pf,Nf);
Gain_real=zeros(Pf,Nf);
DE_real=zeros(Pf,Nf);
PAE_real=zeros(Pf,Nf);
phase_PA_driver=zeros(Pf,Nf);
phase_PA=zeros(Pf,Nf);
%
for i=1:Nf
...
hold on
for i=1:Nf
    plot(Pout_real_PA(:,i),DE_real(:,i),linestyle1(i,:),'linewidth',1)
end
% for i=1:Nx
...
hold on
for i=1:Nf
    plot(Pout_real_PA(:,i),Gain_real(:,i),linestyle1(i,:),'linewidth',1)
end
% for i=1:Nx
%     plot(Pout_dB_mark(:,i),eff_mark(:,i),linestyle1(i,2:3),'linewidth',2);
%     plot(Pout_dB(:,i),eff(:,i),linestyle1(i,1:2),'linewidth',2);
...
hold on
for i=1:Nf
    plot(Pout_real_PA(:,i),PAE_real(:,i),linestyle1(i,:),'linewidth',1)
end
% for i=1:Nx
%     plot(Pout_dB_mark(:,i),eff_mark(:,i),linestyle1(i,2:3),'linewidth',2);
%     plot(Pout_dB(:,i),eff(:,i),linestyle1(i,1:2),'linewidth',2);
...
hold on
for i=1:Nf
    plot(Pout_real_PA(:,i),Im_real(:,i),linestyle1(i,:),'linewidth',2);
end
set(gcf,'color','w');

for i=1:Nf
 plot(Pout_real_PA(:,i),Ia_real(:,i),linestyle1(i+1,1:2),'linewidth',2);
end
h=legend('Main','Aux');
% set(h,'fontsize',14,'fontname','Times New Roman')
% title('DE vs Normalized Pout for various x_n','fontsize',14,'fontname','Times New Roman');
...
for i=1:1:Nf
for j=1:Pf
    phase_S21_PA(j,i)=phase_PA_driver(j,i)-Phase_Driver(j,i);
end
end
for i=1:1:Nf
for j=1:Pf
    phase_S21_PA_nomalized(j,i)=phase_S21_PA(j,i)-phase_S21_PA(1,i);
   
end
end
for i=1:Nf
 plot(Pout_real_PA(:,i),phase_S21_PA_nomalized(:,i),linestyle1(i+1,1:2),'linewidth',2);
end
h=legend('AM-PM');
% set(h,'fontsize',14,'fontname','Times New Roman')
% title('DE vs Normalized Pout for various x_n','fontsize',14,'fontname','Times New Roman');
