...
load('driver_data_for_SLCG_1G_2G_2023-04-10-13-46.mat');
f_start_D =1e9;
f_stop_D =2e9;
%
p_min_D = -35;
...
%     if freq==1e9
%         fun_PowerSetting_HMP2030(HMP2030_IP,HMP_channel,5.8);
%     elseif freq > 1e9 && freq <= 1.3e9
%         fun_PowerSetting_HMP2030(HMP2030_IP,HMP_channel,5.2);
%     elseif freq > 1.3e9 && freq <= 1.4e9
%         fun_PowerSetting_HMP2030(HMP2030_IP,HMP_channel,5.2);
%     elseif freq >= 1.5e9 && freq <= 1.6e9
%         fun_PowerSetting_HMP2030(HMP2030_IP,HMP_channel,5);
%     elseif freq > 1.6e9 && freq <= 1.8e9
%         fun_PowerSetting_HMP2030(HMP2030_IP,HMP_channel,4.8);
%     else
%         fun_PowerSetting_HMP2030(HMP2030_IP,HMP_channel,4.6);
%     end
    %fprintf('Attenuation at %g Hz is %g dB\n', freq, Mea_Attenuator(freq));
    %PM.offset(0);
        if freq==1e9
        Voltage_Setting_DP821A(DP821A_IP,5.8,2);
    elseif freq > 1e9 && freq <= 1.3e9
        Voltage_Setting_DP821A(DP821A_IP,5.6,2);
    elseif freq > 1.3e9 && freq <= 1.4e9
        Voltage_Setting_DP821A(DP821A_IP,5.0,2);
    elseif freq >= 1.5e9 && freq <= 1.5e9
        Voltage_Setting_DP821A(DP821A_IP,5,2);
    elseif freq >= 1.6e9 && freq <= 1.8e9
        Voltage_Setting_DP821A(DP821A_IP,4.6,2);
    else
        Voltage_Setting_DP821A(DP821A_IP,4.6,2);
    end
    ...
    for p_target=p_min:p_step:p_max
            ...
            for v_target=v_start:v_step:v_stop
                v_idx =1+(v_target-v_start)/v_step;
                Voltage_Setting_DP821A(DP821A_IP,v_target,1);
        % Pin=p_target+32;     
...
%              end
            ...
            Pout=p_out-Mea_Attenuator_SLCG(freq);
            fprintf('SG power is %02.2f ', Pout);
            gain=Pout-Pin;
            P_DC=V_m*(I_m-0.001)+14*I_a;
            DE=100*10^((Pout-30)/10) / P_DC;
            PAE=100*(10^((Pout-30)/10)-10^((Pin-30)/10))/ P_DC;
            
            % create the data table
        data(f_idx).table(v_idx,1) = v_target;     % signal generator power level
        data(f_idx).table(v_idx,2) = p_in;     % PA_in (a.k.a Driver_out)
        data(f_idx).table(v_idx,3) = V_m;      % main voltage
        data(f_idx).table(v_idx,4) = I_m;      % main current
        data(f_idx).table(v_idx,5) = I_a;
        data(f_idx).table(v_idx,6) = P_DC;      % DC Power
        data(f_idx).table(v_idx,7) = p_out;    % PA_out ;rom PM
        data(f_idx).table(v_idx,8) = Pout;    % PA_out real
        data(f_idx).table(v_idx,9) = gain;     % gain
        %data(f_idx).table(p_idx,7) = V_a;      % auxiliary voltage
        %data(f_idx).table(p_idx,8) = I_a;      % auxiliary current
        data(f_idx).table(v_idx,10) = PAE;      % PAE
        data(f_idx).table(v_idx,11) = DE;      % drain efficiency    
            
               end
    end
     
             %SignalGenerator_SMW200A(SMWAdd,freq,p_target,3);
            Voltage_Setting_DP821A(DP821A_IP,28,1)
 ...
 data_file_name = [ 'PA_SLCG_voltage_sweep_1_0_2_0G_' datestr(now,'yyyy-mm-dd-HH-MM' )];
...
%         Att_out(f_idx)=Mea_Attenuator_2(freq); 
    ...
    Att_out(i)=Mea_Attenuator_SLCG(freq(i)); 
end
...
set(gcf,'color','w');
axis([20,42,0,80]);
h=legend('1GHz','1.1GHz','1.2GHz','1.3GHz','1.4GHz','1.5GHz','1.6GHz','1.7GHz','1.8GHz','1.9GHz','2GHz');
