...
load('driver_data_TL1G18G2023-04-08-12-19.mat');
f_start_D =4.2e9;
f_stop_D =6.8e9;
%
p_min_D = -40;
...
for freq=f_start:f_step:f_stop
 ...
 data_file_name = [ 'PA_Doherty_linear_data_CW' datestr(now,'yyyy-mm-dd-HH-MM' )];
...
for i=1:Nf
    ...
    Psat(i)=max(data(i).table(:,8));
    PAEsat(i)=max(data(i).table(:,10));
end 
...
hold on
for i=1:Nf
    plot(data(i).table(:,8),data(i).table(:,11),linestyle1(i,:),'linewidth',1)
end
% for i=1:Nx
...
set(gcf,'color','w');
axis([20,40,0,70]);
h=legend('3.8GHz','4GHz','4.2GHz','4.6GHz','4.8GHz','5GHz','5.2GHz','5.4GHz','5.6GHz','5.8GHz','6GHz','6.2GHz','6.4GHz');
...
hold on
for i=1:Nf
    plot(data(i).table(:,8),data(i).table(:,9),linestyle1(i,:),'linewidth',1)
end
% for i=1:Nx
%     plot(Pout_dB_mark(:,i),eff_mark(:,i),linestyle1(i,2:3),'linewidth',2);
%     plot(Pout_dB(:,i),eff(:,i),linestyle1(i,1:2),'linewidth',2);
...
hold on
for i=1:Nf
    plot(data(i).table(:,8),data(i).table(:,10),linestyle1(i,:),'linewidth',1)
end
% for i=1:Nx
%     plot(Pout_dB_mark(:,i),eff_mark(:,i),linestyle1(i,2:3),'linewidth',2);
%     plot(Pout_dB(:,i),eff(:,i),linestyle1(i,1:2),'linewidth',2);
...
set(gcf,'color','w');
axis([f_start/1e9,f_stop/1e9,37,41]);
...
set(gcf,'color','w');
axis([f_start/1e9,f_stop/1e9,10,70]);
% h=legend('4.9GHz','5.0GHz','5.1GHz');
 set(h,'fontsize',14,'fontname','Times New Roman')
% title('DE vs Normalized Pout for various x_n','fontsize',14,'fontname','Times New Roman');
xlabel('Frequency (GHz)','fontsize',15,'fontname','Times New Roman','fontweight','b');
