...
function[instrumentHandle ]= AWG_N8241A_Setup(Fsample, AWG_gain)
...
%   agt_awg_setstate( instrumentHandle, 'refclksrc', 'ext' );    % 'ext' or 'int'. 10 MHz reference clock source.
  ...
  agt_awg_setstate( instrumentHandle, 'trigthresholdA', 3);
  agt_awg_setstate( instrumentHandle, 'start', 'ext1')
  
  disp('marker enabled');
  %agt_awg_setstate( instrumentHandle, 'mkrsource','wfm_start', 1);
  agt_awg_setstate( instrumentHandle, 'mkrsource','wfm_start', 1);
  agt_awg_setstate( instrumentHandle, 'mkrsource','wfm_start', 2);
