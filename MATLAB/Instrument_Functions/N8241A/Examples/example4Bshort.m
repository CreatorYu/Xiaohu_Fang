...
%
% Create a waveform - a ramp with 2000 pts

numberOfSamples = 1000;
x = 1;
samples = 1:numberOfSamples;
if mod(x,2)==0
    ch1 = round(samples/numberOfSamples);
    if (samples/numberOfSamples ==1)
        x=x+1;
    end
else
    ch1 = floor(samples/numberOfSamples);
    
end    
%ch1 = round(samples/numberOfSamples) ;%(samples/numberOfSamples)*2 - 1;
ch2 = 1 - (samples/numberOfSamples)*2;
waveform = [ ch1; ch2 ];
