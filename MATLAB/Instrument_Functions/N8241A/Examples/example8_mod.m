...
disp('Opening a session to the instrument');
    [instrumentHandle2, errorN, errorMsg ] = agt_awg_open('TCPIP','TCPIP0::169.254.93.209::inst0::INSTR');
        if errorN ~= 0
            disp(errorN);
            disp(errorMsg);
    ...
    [instrumentHandle1, errorN, errorMsg ] = agt_awg_open('TCPIP','TCPIP0::169.254.144.24::inst0::INSTR');
        if errorN ~= 0
            disp(errorN);
            disp(errorMsg);
            disp('program stopped');
