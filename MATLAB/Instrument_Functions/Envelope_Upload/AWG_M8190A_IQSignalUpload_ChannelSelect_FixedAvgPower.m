function AWG_M8190A_IQSignalUpload_ChannelSelect_FixedAvgPower(SignalsCell, ...
    ...
    DisplaySignal,channel,Expansion_Margin, PAPR_input, PAPR_original, I_offset, Q_offset)
...
%     CheckPower(SignalsCell{1}, 1)
    ...
    [mean_power, max_power, papr_power] = checkPower(real(iqtotaldata), imag(iqtotaldata), 1);
...
%     
%     iqtotaldata = setMeanPower(real(iqtotaldata), imag(iqtotaldata), papr_power);
...
%         CheckPower(iqdata, 1)
%     [mean_power, max_power, papr_power] = checkPower(real(iqdata), imag(iqdata), 1);
