function [ kernel , NbCoeff ] = VolterraDpdIdentification_PruneVolterraKernels( VolterraParameters )
    display( 'Reduce number of Coefficients using DDR' ) ; 
    R      = VolterraParameters.DDRorder ;
    kernel = VolterraParameters.Kernel   ;
%% 1st Kernel
    if R == 0
        display( 'Reduction of kernal #1' ) ;
        %tic
        kernel{ 1 } = [ ] ;
...
%% 11th Kernel
    ...
    if ~isempty( kernel{ 11 } )
        display( 'Reduction of kernal #11' ) ;
        %tic
...
%% Number of Coefficient
    display('Calculate Number of Coefficients'); 
    %tic
    NbCoeff = 0 ;
    for i = 1 : 2 : 11
