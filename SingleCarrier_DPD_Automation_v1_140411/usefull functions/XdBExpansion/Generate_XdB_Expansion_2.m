function [ Pr_I , Pr_Q ] = Generate_XdB_Expansion_2 ( In_I , In_Q , GainExpansion , InflectionPoint , Fs )

    avg_in=10*log10(mean((abs(In_I + 1j * In_Q)).^2)/100)+30;
...
%     I_in  = I_in(:,1).' ; Q_in  = Q_in(:,1).' ;
    ...
    avg_out=10*log10(mean((abs(Pr_I + 1j * Pr_Q)).^2)/100)+30;
    offset = 10^((-(avg_out-avg_in))/20);
