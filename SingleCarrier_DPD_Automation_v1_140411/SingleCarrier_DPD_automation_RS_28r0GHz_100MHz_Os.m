...
p_sg=-20;
Fcarrier  = 24e9;       % Center frequency of the modulated signal
FramTime  = 0.02e-3;      % Total frame time for the modulated signal
...
Fsample_desired  = FsampleTx;     % The sampling rate of the DPD modeing. Fsample_desired < FsampleRx
Fs=Fsample_desired;               %
...
IP         = '192.168.1.38';                     % set the address of spectrum analyzer
Freq       = Fcarrier;                                    
RefLev     = -7;
% FsampleRx = 2*FsampleTx;
...
if min_size > round(FramTime*Fsampling_Sig) + 0
    min_size = round(FramTime*Fsampling_Sig) + 0;
end
In_I_beforeDPD = In_I_beforeDPD(1:min_size);
In_Q_beforeDPD = In_Q_beforeDPD(1:min_size);

[UpsampleTx1, DownsampleTx1] = rat(FsampleTx/Fsampling_Sig);
In_I_beforeDPD = resample(In_I_beforeDPD,UpsampleTx1,DownsampleTx1);
In_Q_beforeDPD = resample(In_Q_beforeDPD,UpsampleTx1,DownsampleTx1);
...
[UpsampleTx, DownsampleTx] = rat(FsampleRx/FsampleTx);
...
%         V_a_with_DPD = PS_a.voltage;
%         I_a_with_DPD = PS_a.current;
         Pout_measured_with_DPD = AvgPower- Mea_Attenuator_MMWave_2(Freq)
         pause(1)
...
%         V_a_with_DPD = PS_a.voltage;
%         I_a_with_DPD = PS_a.current;
         Pout_measured_without_DPD = AvgPower- Mea_Attenuator_MMWave_2(Freq);
