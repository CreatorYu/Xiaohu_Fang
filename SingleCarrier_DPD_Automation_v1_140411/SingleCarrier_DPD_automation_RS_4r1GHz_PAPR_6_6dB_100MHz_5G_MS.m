...
p_sg=-5;
Fcarrier  = 5.6e9;       % Center frequency of the modulated signal
FramTime  = 0.05e-3;      % Total frame time for the modulated signal
...
FsampleRx        = 200e6;         % The sampling rate of the receiver (max 160MHz)
...
%DP932U_IP='TCPIP0::192.168.1.105::INSTR';        % set the address of voltage source
IP         = '192.168.1.19';                     % set the address of spectrum analyzer
Freq       = Fcarrier;                                    
RefLev     = -5;
FsampleRx = FsampleTx;
...
[UpsampleTx, DownsampleTx] = rat(FsampleTx/FsampleRx);
    ...
    PlotSpectrum(ResampledRecI, ResampledRecQ, ResampledRecI, ResampledRecQ) ;
%
...
%         V_a_with_DPD = PS_a.voltage;
%         I_a_with_DPD = PS_a.current;
         Pout_measured_with_DPD = meanPower_FSW-Mea_Attenuator(Freq);
...
%     [In_I, In_Q, out_I1, out_Q1]                = UnifyLength(In_I, In_Q, ResampledRecI, ResampledRecQ);
    ...
    N1=length(In_I);
    In_Q0=zeros(N1,1); out_Q0=zeros(N1,1);
    In_I0=zeros(N1,1); out_I0=zeros(N1,1);
    [DelayAdjusted_In_I, DelayAdjusted_In_Q, DelayAdjusted_Out_I, DelayAdjusted_Out_Q, timedelay1] = AdjustDelay(In_I, In_Q, out_I1, out_Q1,Fs,2000) ;
       [DelayAdjusted_In_I1, DelayAdjusted_In_Q1, DelayAdjusted_Out_I1, DelayAdjusted_Out_Q1, timedelay2] = AdjustDelay(In_I, In_Q0, out_I1, out_Q0,Fs,2000) ;
   [DelayAdjusted_In_I2                           , DelayAdjusted_In_Q2, DelayAdjusted_Out_I2, DelayAdjusted_Out_Q2, timedelay3] = AdjustDelay(In_I0, In_Q, out_I0, out_Q1,Fs,2000) ;
 
       PlotGain(DelayAdjusted_In_I1, DelayAdjusted_In_Q1, DelayAdjusted_Out_I1, DelayAdjusted_Out_Q1) ;
     PlotGain(DelayAdjusted_In_I2, DelayAdjusted_In_Q2, DelayAdjusted_Out_I2, DelayAdjusted_Out_Q2) ;
...
%         V_a_with_DPD = PS_a.voltage;
%         I_a_with_DPD = PS_a.current;
         Pout_measured_without_DPD = meanPower_FSW-Mea_Attenuator(Freq);
