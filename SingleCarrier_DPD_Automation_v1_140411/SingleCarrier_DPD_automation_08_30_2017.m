...
Transmitter_type  = 'AWG_N8241A';  % Choose between 'AWG' and 'ESG' for the transmitter - In case of ESG you should use MATLAB 2009a!!!
...
Expansion_Margin = 2.0;           % Used for high speed AWG only. It is used to maintain the average power of AWG when the PAPR of the pre-distorted signal increases.
...
FsampleRx        = 800e6;         % The sampling rate of the receiver (max 160MHz)
...
UXA_Atten             = 23;                                       % The mechanical attenuation in dB for the PXA when dowloading the signal. From 6 to 24 with steps of 2 dB
UXA_ClockReference       = 'External';
%
DAC_SamplingRate            = 625e6;      % The sampling rate of the AWG - The input I/Q files with sampling rate of FsampleTx will be upsampled to this number. DAC_SamplingRate has to be an integer multiple of FsampleTx
RF_channel                  = 1;        % AWG channel used for sending RF signal - Not used in 'ESG' mode
VFS                         = 0.61;      % Full scale voltage of the AWG. 0.1 < VFS < 0.7;
...
%% APPLY CALIBRAITON
    Cal = load('IQ_Imbalance_fLO4r8GHz_Freq_Domain');
...
for IterationCount = 1:NofIteration
    ...
    elseif strcmp(Transmitter_type,'AWG_N8241A')
        ...
        [UpsampleAWG, DownsampleAWG] = rat(FsampleAWG/FsampleTx);
        In_I_AWG = resample(In_I,UpsampleAWG,DownsampleAWG,100);
        In_Q_AWG = resample(In_Q,UpsampleAWG,DownsampleAWG,100);
  %     Waveform = [In_I_AWG' + I_offset; In_Q_AWG' + Q_offset];
 %% Apply calibration
        [I_corr, Q_corr ] = ApplyInverseIQImbalanceFilters(In_I_AWG, In_Q_AWG, FsampleAWG, ...
...
elseif strcmp(Transmitter_type,'AWG_N8241A')
        ...
        instrumentHandle = AWG_N8241A_Setup(FsampleAWG, AWG_Gain);
        [UpsampleAWG, DownsampleAWG] = rat(FsampleAWG/FsampleTx);
        In_I_AWG = resample(In_I_cal,UpsampleAWG,DownsampleAWG,100);
        In_Q_AWG = resample(In_Q_cal,UpsampleAWG,DownsampleAWG,100);
 %      Waveform = [In_I_AWG' + I_offset; In_Q_AWG' + Q_offset];
