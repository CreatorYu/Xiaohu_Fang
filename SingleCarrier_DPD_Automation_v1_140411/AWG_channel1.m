...
FrameTime  = 0.18e-3;      % Total frame time for the modulated signal
...
Expansion_Margin = 1.03+0.55+1.6-0.4;           % Used for high speed AWG only. It is used to maintain the average power of AWG when the PAPR of the pre-distorted signal increases.
...
delay_adjust                  = 1.54e-9;
PA_VS_delay                   = 0*1.35e-9;
...
%  InI_beforeDPD_path = '40MHz_WCDMA_I_Input_PreDist_1.txt';
%  InQ_beforeDPD_path = '40MHz_WCDMA_Q_Input_PreDist_1.txt';
%%%%% WCDMA 10C - 50 MHz
...
PXA_Atten = 16;
mem_truncate = 20;
[xEVM , yEVM, EVM_perc, FsampleDPD, RxPower] = PXA_CaptureResampleAnalyzeEVM(In_I1(mem_truncate:end), In_Q1(mem_truncate:end), Fcarrier, FsampleRx, FsampleTx, FrameTime, PXAAdd, PXA_Atten, 0);
...
% cd('Measurements')
% fidIEH = fopen(['I_no_VS_in_20MHz.txt'],'wt');
% fprintf(fidIEH,'%12.20f\n',real(xEVM));
% fclose(fidIEH);
% fidIEH = fopen(['Q_no_VS_in_20MHz.txt'],'wt');
% fprintf(fidIEH,'%12.20f\n',imag(xEVM));
% fclose(fidIEH);
% fidIEH = fopen(['I_no_VS_out_20MHz.txt'],'wt');
% fprintf(fidIEH,'%12.20f\n',real(yEVM));
% fclose(fidIEH);
% fidIEH = fopen(['Q_no_VS_out_20MHz.txt'],'wt');
% fprintf(fidIEH,'%12.20f\n',imag(yEVM));
% fclose(fidIEH);
% cd ..

cd('Measurements')
fidIEH = fopen(['I_onlyRFPD_in_15MHz_29.9_850MHz.txt'],'wt');
fprintf(fidIEH,'%12.20f\n',real(xEVM));
fclose(fidIEH);
fidIEH = fopen(['Q_onlyRFPD_in_15MHz_29.9_850MHz.txt'],'wt');
fprintf(fidIEH,'%12.20f\n',imag(xEVM));
fclose(fidIEH);
fidIEH = fopen(['I_onlyRFPD_out_15MHz_29.9_850MHz.txt'],'wt');
fprintf(fidIEH,'%12.20f\n',real(yEVM));
fclose(fidIEH);
fidIEH = fopen(['Q_onlyRFPD_out_15MHz_29.9_850MHz.txt'],'wt');
fprintf(fidIEH,'%12.20f\n',imag(yEVM));
fclose(fidIEH);
cd ..
