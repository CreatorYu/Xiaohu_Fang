function [out_Basis, out_MaxM] = Generate_Basis(M, N, model, polyorder, varargin)
        ...
        for t=1:M-1
            for i=0:incr:N-2
                for k=incr:incr:N
                    ...
                    Basis(idx).P.N = i;
                    Basis(idx).P.K = k;
                    Basis(idx).func = @(mem) mem(:,1).*(abs(mem(:,1)).^i).*(abs(mem(:,t+1)).^k);
                end
            end
        end
        ...
        for t=1:M-1
            for i=0:incr:N
                for k=0:incr:N
                    ...
                    Basis(idx).P.N = i;
                    Basis(idx).P.K = k;
                    Basis(idx).func = @(mem) mem(:,t+1).*(abs(mem(:,1)).^i).*(abs(mem(:,t+1)).^k);
                end
            end
        end
    end
