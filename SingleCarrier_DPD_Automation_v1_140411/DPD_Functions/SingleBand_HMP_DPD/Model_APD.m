function [APD_coef, Basis] = Model_APD(APD_in, APD_out, model, params)
...
if model.two_step == 0
    if length(model.engine) > 4 && strcmp(model.engine(1:4), 'Mod_')
        [A, Basis] = Generate_MemPoly_Matrix(x_pd, model.M, model.N, model.engine, model.polyorder, model.FIR_M);
        m_shift = max(model.M, model.FIR_M);
    else
