function [a_coef1 a_coef2]=Identify_dual_band_Volterra_MP(I1_inpa, Q1_inpa,I2_inpa, Q2_inpa, I2_outpa , Q2_outpa ,I3_outpa , Q3_outpa , Dualband_MP_Parameters , rec_srate, NbOfPoint )
...
omega0 = Dualband_MP_Parameters.carrier_frequency;%=2*pi*2.1e9;
seperation = Dualband_MP_Parameters.seperation_btwn_two_bands;%=100e6;
DPD_fs = Dualband_MP_Parameters.DPD_fs;%=100e6/OSR;
...
% N1 = 7;       % Static Polynomial Order - RF1
% N2 = 7;       % Static Polynomial Order - RF2
% M = 3;        % Memory depth
% Mk = 1;        % Memory step

for count1 = 1:NL
    for count2 = 1:NL2
        for count3 = 1:NSupply
            for countm = 0:M-1
                In1_dt = circshift(In1_d,Mk*countm);
                In2_dt = circshift(In2_d,Mk*countm);
                a_dt = abs(In1_dt)+abs(In2_dt);
                a_d = abs(In1_d)+abs(In2_d);
                ...
                U1(:,count1*count2*count3*(countm + 1)) = In1_dt.*(abs(In1_dt)).^(count1 - 1).*(abs(In2_dt)).^(count2 - 1)./((a_dt).^((count3 - 1)));% + In2_dt.*(abs(In2_dt)).^(count1 - 1)./((a_dt).^((count2 - 1)));
                U2(:,count1*count2*count3*(countm + 1)) = In2_dt.*(abs(In2_dt)).^(count1 - 1).*(abs(In1_dt)).^(count2 - 1)./((a_dt).^((count3 - 1)));% + In2_d.*(abs(In2_d)).^(count1 - 1).*(abs(In1_d)).^(count2 - 1);
            end
        end
    end    
end
...
x_in1=x_in1(1+M*Mk:N);
y_out1=y_out1(1+M*Mk:N);
x_in2=x_in2(1+M*Mk:N);
y_out2=y_out2(1+M*Mk:N);
...
size(U1)
size(y_out1)
a_coef1 = (pinv(U1, eps) * [y_out1]); 
a_coef2 = (pinv(U2, eps) * [y_out2]); 
...
%                 size(In1_dt)
                ...
                A1(:,count1*count2*count3*(countm + 1)) = In1_dt.*(abs(In1_dt)).^(count1 - 1).*(abs(In2_dt)).^(count2 - 1)./((a_dt).^((count3 - 1)));% + In2_dt.*(abs(In2_dt)).^(count1 - 1)./((a_dt).^((count2 - 1)));
                A2(:,count1*count2*count3*(countm + 1)) = In2_dt.*(abs(In2_dt)).^(count1 - 1).*(abs(In1_dt)).^(count2 - 1)./((a_dt).^((count3 - 1)));% + In2_d.*(abs(In2_d)).^(count1 - 1).*(abs(In1_d)).^(count2 - 1);
...
% y_model1=zeros(length(y_out1),1);
% y_model2=zeros(length(y_out2),1);
% y_model_1_2=[y_model1 ; y_model2];
% if strcmp(Validation, 'same data') %Validation with the same data set
%  y_model_1_2 = (A * a_coef);      % Calculation of modelled values
% elseif strcmp(model, 'another data')%Validation with different set of data
% end
% y_model1=y_model_1_2(1:length(y_model1),1);
% y_model2=y_model_1_2(length(y_model1)+1:length(y_model1)+length(y_model2),1);
y_model1=(A1 * a_coef1);
y_model2=(A2 * a_coef2);
...
V1_in    = x_in1((1+M*Mk:N):end)     ;
V2_in    = x_in2((1+M*Mk:N):end)   ;
V2_out   = y_out1((1+M*Mk:N):end)   ;
V3_out   = y_out2((1+M*Mk:N):end)    ;
V2_outNN = y_model1(1+M*Mk:N);
V3_outNN = y_model2(1+M*Mk:N);
