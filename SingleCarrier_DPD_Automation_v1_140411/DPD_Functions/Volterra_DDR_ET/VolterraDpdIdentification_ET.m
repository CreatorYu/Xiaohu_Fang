function [ VolterraParameters , VolterraCoeff, VolterraOutput , StaticOutput , NMSE ] = VolterraDpdIdentification_ET ( Pr_I , Pr_Q , Out_I , Out_Q , Vdd, VolterraParameters , NbOfPoint , DPD)
...
%     DPD = false ;
    PS  = NbOfPoint ;
	CDTRFTB = 1+0*1e2 ;
	[ In_I , In_Q , Out_I , Out_Q , Vdd_import ] = VolterraDpdIdentification_ImportData_ET( Pr_I , Pr_Q , Out_I , Out_Q , Vdd, PS , CDTRFTB ) ;
...
%         plot(Vdd_import); hold on;
%         plot(abs(data.In)/max(abs(data.In)),'r');
%         
%         dcvdfcdkf 
