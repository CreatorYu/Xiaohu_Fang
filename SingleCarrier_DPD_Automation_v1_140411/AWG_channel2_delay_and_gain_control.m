...
Fcarrier  = 0.8e9;         % Center frequency of the modulated signal
FrameTime  = 0.2e-3;      % Total frame time for the modulated signal
PowerBand = 0;           % Power in dBm for ESG (In case of high speed AWG, the power is controlled using VFS)
BW        = 15e6;        % Bandwidth of the modulated signal - Used to calculated ACLR and ACPR from the downloaded I/Q signals  %to be linked to signal
fG        = 300e3;       % Guard band for the modulated signal - Used to calculated ACLR and ACPR from the downloaded I/Q signals

Expansion_Margin = 1.2;           % Used for high speed AWG only. It is used to maintain the average power of AWG when the PAPR of the pre-distorted signal increases.
...
predistort = 0; %set to 1 if you wanted channel one to transmit a separate predistorted signal
measure    = 0; %set to 1 if you wanted the receiver to measure and download the output signal

%without static predistortion
% V_scaling        = 0.97; %33.6dBm
% V_scaling        = 1.17; %31.9dBm
V_scaling        = 1.45; %30.53dBm
...
% InI_beforeDPD_path = 'LTE_20MHz_In_I_100r0_PAPR_9r3_16QAM_1ms_cut.txt';
% InQ_beforeDPD_path = 'LTE_20MHz_In_Q_100r0_PAPR_9r3_16QAM_1ms_cut.txt';
...
% InI_beforeDPD_path = 'WCDMA3G_2C_11_In_I_100r0_PAPR_7r9_Version1200_1ms.txt';
% InQ_beforeDPD_path = 'WCDMA3G_2C_11_In_Q_100r0_PAPR_7r9_Version1200_1ms.txt';
...
% cd('Measurements')
% fidIEH = fopen(['I_w_VS_in_40MHz_900MHz.txt'],'wt');
% fprintf(fidIEH,'%12.20f\n',real(xEVM));
% fclose(fidIEH);
% fidIEH = fopen(['Q_w_VS_in_40MHz_900MHz.txt'],'wt');
% fprintf(fidIEH,'%12.20f\n',imag(xEVM));
% fclose(fidIEH);
% fidIEH = fopen(['I_w_VS_out_40MHz_900MHz.txt'],'wt');
% fprintf(fidIEH,'%12.20f\n',real(yEVM));
% fclose(fidIEH);
% fidIEH = fopen(['Q_w_VS_out_40MHz_900MHz.txt'],'wt');
% fprintf(fidIEH,'%12.20f\n',imag(yEVM));
% fclose(fidIEH);
% cd ..

cd('Measurements')
fidIEH = fopen(['I_in_15MHz_800MHz.txt'],'wt');
fprintf(fidIEH,'%12.20f\n',real(xEVM));
fclose(fidIEH);
fidIEH = fopen(['Q_in_15MHz_800MHz.txt'],'wt');
fprintf(fidIEH,'%12.20f\n',imag(xEVM));
fclose(fidIEH);
fidIEH = fopen(['I_out_15MHz_800MHz.txt'],'wt');
fprintf(fidIEH,'%12.20f\n',real(yEVM));
fclose(fidIEH);
fidIEH = fopen(['Q_out_15MHz_800MHz.txt'],'wt');
fprintf(fidIEH,'%12.20f\n',imag(yEVM));
fclose(fidIEH);
cd ..
end