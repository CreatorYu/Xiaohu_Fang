...
% Last Modified by GUIDE v2.5 07-Mar-2014 09:27:16
...
elseif (~isempty(strfind(arbConfig.model, 'DUC')) && ...
    ...
    set(handles.editCarrierOffset, 'String', sprintf('%g', 0));
    set(handles.editFc, 'String', sprintf('%g', arbConfig.carrierFrequency));
else
...
if (exist('doCode', 'var'))
    ...
    '    ''modType'', ''%s'', ''oversampling'', %g, ...\n' ...
    '    ''filterType'', ''%s'', ''filterNsym'', %g, ...\n' ...
    '    ''filterBeta'', %g, ''carrierOffset'', %s, ...\n' ...
    '    ''magnitude'', %s, ''quadErr'', %g, ''correction'', %d);\n' ...
    'iqdownload(iqdata, fs, ''channelMapping'', ''%s'', ...\n' ...
    '    ''segmentNumber'', %d, ''marker'', []);\n'], ...
        numSymbols, modTypeList{modTypeIdx}, oversampling, ...
        filterList{filterIdx}, filterNsym, filterBeta, carrierOffset, ...
        magnitudes, quadErr, correction, channelMapping, segmentNum)];
else
