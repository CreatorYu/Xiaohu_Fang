...
% Last Modified by GUIDE v2.5 12-Mar-2014 09:45:39
...
set(handles.editStartFreq, 'String', sprintf('%g', startFreq));
set(handles.editStopFreq, 'String', sprintf('%g', stopFreq));
set(handles.editNotchFreq, 'String', sprintf('%g', notchFreq));
set(handles.editNotchSpan, 'String', sprintf('%g', notchSpan));
...
if (isfield(arbConfig, 'defaultFc'))
    set(handles.editCarrier, 'String', sprintf('%g', arbConfig.defaultFc));
end
...
if (isscalar(value) && value >= 0 && value <= 100000)
    ...
    elseif (numTones == 1)
        ...
        set(handles.editToneSpacing, 'String', sprintf('%g', spacing));
        set(handles.editStartFreq, 'Enable', 'on');
        set(handles.popupmenuPhase, 'Enable', 'on');
    end
    set(hObject,'BackgroundColor','white');
...
if (strcmp(calString, 'Re-Calibrate') == 1)
    ...
    if (~isempty(res))
        ...
        set(handles.pushbuttonCal, 'String', 'Re-Calibrate');
    end
end
...
switch (get(handles.popupmenuCalType, 'Value'))
    case 1  % spectrum analyzer
        res = iqcal('tone', tone, 'offset', freqOffset, 'recalibrate', recal, 'interleaving', interleaving, 'msgbox', hMsgBox);
    case 2  % VSA Software
        res = iqvsamtone('tone', tone, 'fc', freqOffset, 'recalibrate', recal, 'interleaving', interleaving);
...
if (~isempty(vsaApp))
    ...
    vsafunc(vsaApp, 'freq', fc, span, 51201, Agilent.SA.Vsa.WindowType.FlatTop);
...
if (numTones == 0)
    ...
    if (exist('doCode', 'var'))
        ...
        segmentNum = evalin('base', get(handles.editSegment, 'String'));
        iqdata = sprintf(['iqdata = iqnoise(''sampleRate'', %g, ...\n' ...
